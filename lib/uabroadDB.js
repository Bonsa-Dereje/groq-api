...
/** "registration_fee" -> "Registration Fee" for tag display. */
...
/**
 * Deterministically picks up to `count` display-ready "tags" off an
 * already-fetched test_general_info row (the shape getGeneralInfo()/
 * listGeneralInfo() return — i.e. `.description` already parsed from the
 * stored JSON string). No Groq call, no randomness: it walks
 * `description`'s own key order — which is the JSON's own field order,
 * same every time for a given category — and takes the first `count`
 * keys whose value is a plain string/number. Nested objects/arrays (e.g.
 * ACT's `sections`, IELTS's `test_types`) are skipped since they don't
 * collapse into a single readable "label: value" tag.
 *
 * Returns [{ key, label, value }], shorter than `count` if the row has
 * fewer than `count` scalar fields, or [] if generalInfoRow is null/has
 * none — callers should treat an empty array as "no tags to show", not
 * an error.
 */
export function pickGeneralInfoHighlights(generalInfoRow, count = 2) {
  const description = generalInfoRow?.description
  ...
  for (const [key, value] of Object.entries(description)) {
    ...
    if (typeof value === 'object') continue // nested section/array — not a single tag
    picks.push({ key, label: keyToLabel(key), value: String(value) })
  }
  return picks
}
